...
An Owner/Admin opens **Assistant Memory** from the Dashboard, or opens **Ask LineCrew AI** in the lower-right corner and expands **Saved Memories**. They can ask the assistant to remember a company workflow or prepare a reminder for the currently selected job. The request is parsed deterministically and shown as a **Not saved yet** proposal. The assistant does not save anything itself; the Owner/Admin must choose **Save Reminder** or **Save Workflow Memory**.
...
This release intentionally has no Assistant left-navigation item, Edit button, date/time scheduler, attachment field or visible audit-detail screen. Saved cards show title, instruction, company/job scope and trigger. Job reminders can be marked complete, and every memory can be removed. To change one, remove it and save a corrected proposal. Memory-management questions use a deterministic server answer derived from the actual active memory rows so the AI cannot invent navigation or controls.
