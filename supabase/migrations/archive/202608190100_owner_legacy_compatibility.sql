-- Generated Owner compatibility layer for legacy LineCrew Pro RPCs.
-- Keeps existing behavior while treating Owner as an Admin-equivalent where legacy functions authorize by role.
-- New role-management functions are intentionally excluded.
...
begin
  ...
  if v_company_id is null or v_active is not true or
     v_role not in ('admin', 'gf', 'owner') then
...
begin
  ...
  where dr.company_id = v_company_id
    and (v_role in ('admin', 'gf', 'owner') or dr.created_by = auth.uid())
  group by dr.id;
end;
$$;
...
begin
  ...
  if v_role not in ('foreman', 'gf', 'admin', 'owner') then
    raise exception using
      errcode = '42501',
...
begin
  ...
  if v_company_id is null or
     lower(coalesce(v_role, '')) not in ('admin','owner') or
     v_profile_active is not true then
    raise exception using
      errcode = '42501',
...
begin
  ...
  if v_active is not true or v_company_id is null or
     ...
     (v_role not in ('admin', 'gf', 'owner') and v_created_by <> auth.uid()) then
...
begin
  ...
  if v_attachment.uploaded_by <> auth.uid()
     and lower(coalesce(v_profile.role, '')) not in ('admin','gf', 'owner') then
...
begin
  ...
  if v_company_id is null or v_role not in ('foreman', 'gf', 'admin', 'owner') then
    raise exception using errcode = '42501',
...
begin
  ...
  where p.company_id = v_company_id
    and lower(coalesce(p.role, 'foreman')) in ('foreman', 'gf', 'admin', 'owner')
  order by
