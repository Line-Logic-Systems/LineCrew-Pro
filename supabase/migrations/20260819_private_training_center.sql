...
-- Subscriber-only Training Center foundation.
-- Video objects stay private in Supabase Storage and are exposed with short-lived
-- signed URLs only after the app verifies company subscription + role access.
...
-- Storage reads stay private. The frontend should request signed URLs through
-- the Supabase client only after current_training_access() passes.
