-- Keep the existing digital JSA workflow compatible with Owner/Superintendent roles
-- and keep uploaded company JSAs out of the legacy digital-JSA list.
...
begin
  ...
  if v_role = 'superintendent'
    ...
    raise exception using errcode = '42501',
      message = 'Safety Records permission is disabled for this Superintendent.';
  end if;

  if v_method not in ('digital', 'both') then
    raise exception using errcode = '42501',
      message = 'Digital JSAs are disabled in Company Settings.';
  end if;
...
revoke all on function public.create_standalone_jsa(uuid,date,text,text,text,text,text,text,text,text,text,boolean) from public, anon;
...
begin
  ...
  where safety.company_id = v_company_id
    ...
    and (
      v_role in ('gf', 'superintendent', 'admin', 'owner')
