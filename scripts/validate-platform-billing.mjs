...
function parseInlineScripts(name, html) {
  ...
  for (const [index, script] of scripts.entries()) {
    try {
      ...
      throw new Error(`${name} inline JavaScript does not parse: ${error.message}`);
    }
  }
}
...
const requiredBillingMarkers = [
  'my_company_billing_summary',
  'create-billing-checkout',
  'create-billing-portal',
  'stripe_subscription_linked',
];
for (const marker of requiredBillingMarkers) {
...
for (const marker of requiredSqlMarkers) {
  if (!migration.toLowerCase().includes(marker.toLowerCase())) {
    throw new Error(`billing migration is missing ${marker}`);
  }
}

if (!checkout.includes('BILLING_PLAN_PRICE_MAP')) {
  throw new Error('Checkout must use the server-side BILLING_PLAN_PRICE_MAP.');
}
if (checkout.includes('BILLING_ALLOWED_PRICE_IDS')) {
  throw new Error('Checkout still contains the older client-selected Price ID allowlist path.');
}
if (!checkout.includes('planPriceMap.get(planCode)')) {
  throw new Error('Checkout must bind the company assigned plan to its Stripe Price server-side.');
}
if (!checkout.includes('String(profile.role).toLowerCase() !== "admin"')) {
  throw new Error('Checkout must require company Admin role.');
}
if (!checkout.includes('already has a Stripe subscription')) {
  throw new Error('Checkout must guard against duplicate live subscriptions.');
}
if (!portal.includes('String(profile.role).toLowerCase() !== "admin"')) {
  throw new Error('Billing portal must require company Admin role.');
}
if (!portal.includes('/billing.html?billing=portal-return')) {
  throw new Error('Billing portal must return to the contractor billing page.');
}
if (!webhook.includes('stripe-signature') || !webhook.includes('verifyStripeSignature')) {
  throw new Error('Stripe webhook signature validation is missing.');
}
if (!webhook.includes('eventInsertError.code !== "23505"')) {
  throw new Error('Stripe webhook must recognize unique-event retries by PostgreSQL error code.');
}
if (!webhook.includes('prior?.processed_at')) {
  throw new Error('Stripe webhook must distinguish completed duplicates from retryable failed events.');
}
...
if (owner.includes('SUPABASE_SERVICE_ROLE_KEY') || billing.includes('SUPABASE_SERVICE_ROLE_KEY')) {
  throw new Error('A browser page references the Supabase service-role key.');
}
if (owner.includes('STRIPE_SECRET_KEY') || billing.includes('STRIPE_SECRET_KEY')) {
  throw new Error('A browser page references the Stripe secret key.');
}

console.log('PASS: platform owner and subscription billing validation passed.');
