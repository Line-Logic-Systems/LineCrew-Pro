...
const helperNames=['uniqueOfflineJsaJobs','offlineJsaNetworkFailure','companyAccessInactive','firstStackFrame','desktopViewEnabled','currentErrorPage','formatTeamRole','formatAuditTimestamp','formatCurrency','csvCell','companyLogoExtension','companyJsaFileKey','safeJsaFilename'];
...
for(const [input,expected] of [[null,'jsa-file'],['','jsa-file'],['JSA Page 1.jpg','JSA-Page-1.jpg'],['../bad/name?.pdf','..-bad-name-.pdf'],['---','jsa-file'],['a'.repeat(150)+'.jpg',( 'a'.repeat(136)+'.jpg')]]) {
  const actual=core.safeJsaFilename(input);
  if(actual!==expected) throw new Error(`safeJsaFilename(${JSON.stringify(input)}) returned ${JSON.stringify(actual)}; expected ${JSON.stringify(expected)}.`);
}
...
for(const signature of ['function uniqueOfflineJsaJobs(jobs){','function offlineJsaNetworkFailure(error){','function companyAccessInactive(error){','function firstStackFrame(error){','function desktopViewEnabled(){','function currentErrorPage(){','function formatTeamRole(role){','function formatAuditTimestamp(value){','function formatCurrency(value){','function csvCell(value){','function companyLogoExtension(file){','function companyJsaFileKey(file){','function safeJsaFilename(name){']) if(!index.includes(signature)) throw new Error(`Legacy inline app-core fallback missing: ${signature}`);

console.log('App core modularization guard passed.');
console.log('- low-risk shared helpers match legacy behavior, including safe JSA filenames');
