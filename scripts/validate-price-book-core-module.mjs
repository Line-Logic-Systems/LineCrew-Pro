...
const helperNames = ['normalizeImportHeader','importEditDistance','importHeaderMatchConfidence','normalizedPriceWorkType','importCell','importPrice','numericImportPrice','spreadsheetColumnName'];
...
for (const [index, expected] of [[0,'A'],[1,'B'],[25,'Z'],[26,'AA'],[27,'AB'],[51,'AZ'],[52,'BA'],[701,'ZZ'],[702,'AAA']]) {
  const actual = core.spreadsheetColumnName(index);
  if (actual !== expected) throw new Error(`spreadsheetColumnName(${index}) returned ${actual}; expected ${expected}.`);
}
...
console.log('- import parsing, price validation, work-type and spreadsheet-column behavior match legacy behavior');
